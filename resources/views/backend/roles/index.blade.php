...
@section('subheader-btn')
  @if (Sentinel::getUser()->hasAccess(['roles.add']))
...
@endif
@if (Sentinel::getUser()->hasAccess(['roles.updt']))
...
<div class="kt-portlet kt-portlet--mobile">
  <div class="kt-portlet__head kt-portlet__head--lg">
    <div class="kt-portlet__head-label">
      ...
      <h3 class="kt-portlet__head-title">
        @lang('global.app_list') @lang('role.r_title')
      </h3>
    </div>
  ...
  <div class="kt-portlet__body">
    ...
    <table class="table table-striped- table-bordered table-hover table-checkable" id="tbl_roles">
      <thead>
        <tr>
          <th>@lang('role.r_field.r_slug')</th>
          <th>@lang('role.r_field.r_name')</th>
...
<script type="text/javascript">
      ...
      columnDefs: [
        {
          ...
          render: function(data, type, full, meta) {
            ...
            return ``+
              @if(Sentinel::getUser()->hasAccess(['roles.updt']))
                   ` <a class="btn btn-success btn-sm" href="javascript::void(0)" onclick="edit(`+data+`,'`+full.slug+`','`+full.name+`')">@lang('global.app_edit') {{$name}}</a>`+
              @endif
              @if(Sentinel::getUser()->hasAccess(['roles.permissions']))
                   ` <a class="btn btn-success btn-sm" href="`+base+`/roles/`+data+`/permission">@lang('permission.p_title') {{$name}}</a>`+
              @endif
              @if(Sentinel::getUser()->hasAccess(['roles.destroy']))
  ...
  function delete_role(id) {
    swal.fire({
        ...
        if (result.value) {
            ...
            $('#form-delete').attr('action', "{{route('roles.index')}}/"+id);
            $('#form-delete').submit();
  ...
  function edit(id, slug, name) {
    if($('#edit').is(":visible") && id==id_awal){
      ...
      $('#e_slug').val(slug);
      $('#e_name').val(name);
      $('#form-update').attr('action', "{{route('roles.index')}}/"+id);
    }
